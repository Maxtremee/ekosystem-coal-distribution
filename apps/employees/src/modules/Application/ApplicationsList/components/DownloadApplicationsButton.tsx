...
export default function DownloadApplicationsButton() {
  const { values } = useFilteringContext();
  const { mutate, isLoading } = trpc.applications.downloadFiltered.useMutation({
    onSuccess: (res) => downloadCsv(res, "wnioski"),
  });
